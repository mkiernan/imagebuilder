...
az identity create -g $gridResourceGroup -n $identityName
...
sed -i -e "s/<rgName>/$gridResourceGroup/g" aibRoleImageCreation.json
...
rt="[]"
while [[ "$rt" == "[]" ]]; do
   rt=$(az role definition list -g $gridResourceGroup --name $imageRoleDefName)
   sleep 30
done
...
az sig create -g $gridResourceGroup --gallery-name $sigName
...
sed -i -e "s/<rgName>/$gridResourceGroup/g" RHELImageTemplateSIG.json
